...
namespace litl
{
     ...
     *
     *     #define LITL_LOG_TRACE(...) litl::Logger::trace(__VA_ARGS__)
     *
    ...
    {
        litl::Logger::trace(std::forward<Args>(args)...);
    }

    template<typename... Args>
    inline void logDebug(Args&&... args)
    {
        litl::Logger::debug(std::forward<Args>(args)...);
    }

    template<typename... Args>
    inline void logInfo(Args&&... args)
    {
        litl::Logger::info(std::forward<Args>(args)...);
    }

    template<typename... Args>
    inline void logWarning(Args&&... args)
    {
        litl::Logger::warning(std::forward<Args>(args)...);
    }

    template<typename... Args>
    inline void logError(Args&&... args)
    {
        litl::Logger::error(std::forward<Args>(args)...);
    }

    template<typename... Args>
    inline void logCritical(Args&&... args)
    {
        litl::Logger::critical(std::forward<Args>(args)...);
    }
}

// Macro here only for file/line capture. Use other options if file/line capture is not needed.

#define LITL_LOG_TRACE_CAPTURE(...)    litl::Logger::trace(__VA_ARGS__, litl::Logger::formatFileCapture(__FILE__, __LINE__))
#define LITL_LOG_DEBUG_CAPTURE(...)    litl::Logger::debug(__VA_ARGS__, litl::Logger::formatFileCapture(__FILE__, __LINE__))
#define LITL_LOG_INFO_CAPTURE(...)     litl::Logger::info(__VA_ARGS__, litl::Logger::formatFileCapture(__FILE__, __LINE__))
#define LITL_LOG_WARNING_CAPTURE(...)  litl::Logger::warning(__VA_ARGS__, litl::Logger::formatFileCapture(__FILE__, __LINE__))
#define LITL_LOG_ERROR_CAPTURE(...)    litl::Logger::error(__VA_ARGS__, litl::Logger::formatFileCapture(__FILE__, __LINE__))
#define LITL_LOG_CRITICAL_CAPTURE(...) litl::Logger::critical(__VA_ARGS__, litl::Logger::formatFileCapture(__FILE__, __LINE__))
