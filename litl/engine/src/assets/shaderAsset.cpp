...
namespace litl
{
        ...
        [[nodiscard]] bool decodeNonLitShaderBytes(ShaderAsset* shaderAsset, AssetRegistration const& assetRegistration, std::span<std::byte const> otherBytes, AssetErrorCode& error) noexcept
        {
            ...
            if (importResult.success)
            {
                ...
                if (importedItem.getType() == import::ImportedDataType::Shader)
                {
                    ...
                    if (shader != nullptr)
                    {
                        shaderAsset->shaderIntermediateData = shader->intermediateShader;
                        return true;
                    }
                    else
                    {
                        logError("Unexpected null imported shader in asset decode.");
    ...
    bool ShaderAsset::processOnMain(Asset* asset, AssetRegistration const& assetRegistration, AssetManager& assetManager, ObjectPool& objectPool, AssetErrorCode& error) noexcept
    {
        ShaderAsset* shaderAsset = static_cast<ShaderAsset*>(asset);
