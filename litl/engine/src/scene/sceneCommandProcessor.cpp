...
namespace litl
{
    ...
    void SceneCommandProcessor::onChangeArchetype(Scene& scene, World& world, EntityChange const& change) const noexcept
    {
        ...
        bool prevHadBounds = prevArchetype->hasComponent<Bounds>();

        bool currHasTransform = currArchetype->hasComponent<Transform>();
        bool currHasBounds = currArchetype->hasComponent<Bounds>();
        ...
        else if (prevHadBounds && !currHasBounds)
        {
            // Lost the Bounds component. Need to inform the Scene Partition.
            // ... todo ...
        }
        else if (!prevHadTransform && currHasTransform)
        {
            auto record = world.getEntityRecord(change.entity);

            // Gained the Transform component. Start tracking it.
            if (currHasBounds)
            {
                scene.add(change.entity, currArchetype->getComponent<Transform>(record), {});
            }
            else
            {
