...
* Import library (`litl-import`) which transforms external formats to their internal binary file formats. For example, `.obj`/`.fbx`/`.glTF`/`.glb` → `.litlmesh`.
...
* It does not have any normals and so those are generated for it during the conversion to a `.litlmesh`, along with other not-quite-as-visible mesh operations.
